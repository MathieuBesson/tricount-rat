...
export class UserService extends RootService {
    ...
    public async create(createUserDto: CreateUserDto): Promise<ConfirmationActionMessage> {
        ...
        // Create user in BDD
        await this.prismaService.user.create({ data: { firstName, lastName, email } })
        return { message: `${this.getEntityNameCapitalized()} created` }
    }

    public async update(id: number, updateEntityDto: UpdateUserDto): Promise<ConfirmationActionMessage> {
        ...
        // Update user
        await this.prismaService.user.update({ where: { id }, data: { ...updateEntityDto } })
